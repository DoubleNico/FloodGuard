<template>
  <div class="space-y-6 max-w-[1400px] mx-auto">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-(--label-text) tracking-tight">Alerts & Broadcast</h1>
        <p class="text-sm text-(--hint-text) mt-1">Manage flood alerts and send emergency broadcasts</p>
      </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <StatCard icon="mdi:bell-ring" label="Active Alerts" :value="activeAlerts.length" glow-color="danger" icon-bg="#EF4444" />
      <StatCard icon="mdi:clock-check" label="Pending Review" :value="pendingReview.length" glow-color="warning" icon-bg="#F59E0B" />
      <StatCard icon="mdi:broadcast" label="Broadcasts Sent" :value="publishedAlerts.length" glow-color="success" icon-bg="#22C55E" />
      <StatCard icon="mdi:account-group" label="Total Notified" :value="totalRecipients" glow-color="info" icon-bg="#3B82F6" />
    </div>
    ...
    <Tabs v-model="activeTab" default-value="all">
      ...
      <TabsContent value="all">
        <div class="space-y-3 mt-4">
          <AlertRow v-for="alert in alerts" :key="alert.id" :alert="alert" @broadcast="onBroadcast" @update-status="onUpdateStatus" />
        </div>
      </TabsContent>
      <TabsContent value="published">
        <div class="space-y-3 mt-4">
          <AlertRow v-for="alert in publishedAlerts" :key="alert.id" :alert="alert" @broadcast="onBroadcast" @update-status="onUpdateStatus" />
        </div>
      </TabsContent>
      <TabsContent value="review">
        <div class="space-y-3 mt-4">
          <AlertRow v-for="alert in pendingReview" :key="alert.id" :alert="alert" @broadcast="onBroadcast" @update-status="onUpdateStatus" />
        </div>
      </TabsContent>
      <TabsContent value="closed">
        <div class="space-y-3 mt-4">
          <AlertRow v-for="alert in closedAlerts" :key="alert.id" :alert="alert" @broadcast="onBroadcast" @update-status="onUpdateStatus" />
        </div>
      </TabsContent>
    </Tabs>
    ...
    <Modal v-model="showCreateModal" title="Create New Alert" size="lg">
      <Form class="space-y-4" @submit="handleCreateAlert">
        <CustomSelect v-model="newAlert.type" label="Alert Type" :options="alertTypeOptions" :required="true" />
        <CustomSelect v-model="newAlert.severity" label="Severity" :options="severityOptions" :required="true" />
        <Input v-model="newAlert.title" label="Alert Title" placeholder="e.g. Danube flood warning for Port area" :required="true" />
        <Textarea v-model="newAlert.message" label="Message" placeholder="Detailed alert message for citizens..." :rows="4" :required="true" />
        <CustomSelect v-model="newAlert.affectedAreas" label="Affected Areas" :options="galatiZones.map(z => ({ label: z, value: z }))" :required="true" />
      </Form>
      <template #footer="{ close }">
        <Button variant="outline" color="secondary" @click="close">Cancel</Button>
        <Button variant="solid" color="primary" @click="handleCreateAlert">Create Alert</Button>
      </template>
    </Modal>

    <Modal v-model="showBroadcastModal" title="Broadcast Global Alarm" size="md">
      <div class="text-center space-y-4">
        <div class="h-20 w-20 mx-auto rounded-full bg-red-500/10 flex items-center justify-center alarm-ring">
...
const { alerts, activeAlerts, publishedAlerts, pendingReview, totalRecipients, galatiZones, createAlert, updateAlertStatus, broadcastGlobalAlarm, severityLabel } = useAlerts();
...
const handleCreateAlert = () => {
  ...
  createAlert({
    ...
    affectedAreas: newAlert.affectedAreas ? [newAlert.affectedAreas] : ["Centru"],
