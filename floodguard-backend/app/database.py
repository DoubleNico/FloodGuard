...
def create_schema(conn: sqlite3.Connection) -> None:
    conn.executescript(
        ...
        CREATE TABLE IF NOT EXISTS forecast_snapshots (
            ...
            inundation_geojson TEXT
        );

        CREATE INDEX IF NOT EXISTS idx_forecast_snapshots_site_time
            ON forecast_snapshots(site_id, generated_at DESC);
...
def seed_database(conn: sqlite3.Connection) -> None:
    ...
    conn.executemany(
        ...
        """,
        [
            ("usr_001", "operator", _demo_password_hash("password123"), "operator", "FloodGuard Operator"),
            ("usr_003", "admin", _demo_password_hash("password123"), "admin", "FloodGuard Admin"),
        ],
    )
    ...
    _seed_demo_sites(conn, now)


def _seed_demo_sites(conn: sqlite3.Connection, now: str) -> None:
    ...
    default_thresholds = json.dumps(
        {
            ...
            "glofas_probability": 0.4,
        }
    )
    conn.executemany(
        """
        INSERT INTO sites (
            name, country, lat, lng, footprint_geojson, thresholds_json, contacts_json, created_at, updated_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        [
            (
                "Galați Refinery North",
                "RO",
                45.45,
                28.05,
                None,
                default_thresholds,
                json.dumps(["andrei.ionescu@floodguard.com"]),
                now,
                now,
            ),
            (
                "Constanța Industrial Port",
                "RO",
                44.17,
                28.65,
                None,
                default_thresholds,
                json.dumps(["safety@floodguard.com"]),
                now,
                now,
            ),
        ],
...
def _demo_password_hash(password: str) -> str:
    ...
    return hashlib.sha256(f"floodguard-demo:{password}".encode("utf-8")).hexdigest()
