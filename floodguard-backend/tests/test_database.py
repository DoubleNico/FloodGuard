...
def test_initialize_database_seeds_core_data(tmp_path: Path) -> None:
    settings = Settings(database_path=str(tmp_path / "floodguard-test.db"))
...
def test_demo_auth_hash_and_token() -> None:
    ...
    token = _create_token({"id": "usr_001", "username": "operator", "role": "operator"}, "secret")
